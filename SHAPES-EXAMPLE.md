...
You will then have to select the relevan Approov SDK version you wish to use. To do so, select the `Exact Version` option and enter the relevant SDK version, in this case we will use the `3.0.0` Approov SDK:
...
```
Comment out the above code and uncomment the one just bellow using Approov and remember to use the actual configuration string for your account. The Approov SDK needs a configuration string to identify the account associated with the app. You will have received this in your Approov onboarding email (it will be something like `#123456#K/XPlLtfcwnWkzv99Wj5VmAxo4CrU267J1KlQyoz8Qo=`).
```swift
func initializeSession(){
    if (session == nil) {
        // *** COMMENT OUT IF USING APPROOV APPROOV
