...
Enter the repository `https://github.com/approov/approov-service-alamofire.git` into the search box. You will then have to select the relevant version you wish to use. To do so, select the `Exact Version` option and the latest version available should be selected for you; if that is not the case, enter `3.1.2-signed`
...
Before using Approov you need to import the Alamofire Service. In the `ViewController.swift` source file uncomment the lien to import the Approov module:
...
Lastly, please ensure you point the shapes request to the endpoint that checks the validity of the `Approov-Token` header, `https://shapes.approov.io/v3/shapes/`. Uncomment the line below (commenting out the previous definition):
...
If you build and run the app now, you should get a `400` response from the shapes endpoint, since it expects an authenticated application in order to provide a valid shape. In order authenticate the application, you should register it with the Approov service.
...
This section provides an illustration of an alternative option for Approov protection if you are not able to modify the backend to add an Approov Token check. We are still going to be using `https://shapes.approov.io/v1/shapes/` that simply checks for an API key, so please change back the code to:
 
```swift
static let currentShapesEndpoint = "v1"
```
 
The `apiSecretKey` variable also needs to be changed as follows, removing the actual API key out of the code. Uncomment the line below (commenting out the previous definition):
